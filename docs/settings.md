...
|---|---|---|
| `WA_AUTH_TOKEN` | — | Bearer token for the UI and MCP. Anyone holding it can read and send on your account. `MCP_AUTH_TOKEN` is accepted as an alias. |
...
| `WA_OAUTH` | `1` | Off falls back to the token in the URL. |
